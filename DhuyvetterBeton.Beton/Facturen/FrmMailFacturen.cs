...
{
    public partial class FrmMailFacturen : DevExpress.XtraBars.FluentDesignSystem.FluentDesignForm
    {
        string pdfLocatieMail = "";
        List<Klant> klantenLijst = new List<Klant>();
        List<Klant> KlantenZondermail;
        string USER;
        public FrmMailFacturen(string user1)
        {
            USER = user1;
            InitializeComponent();
        }

        private void FrmMailFacturen_Load(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            List<Klant> klantenLijst = Klant.KrijgAlleKlanten();
            klantenLijst.Sort((X, Y) => X.Naam.CompareTo(Y.Naam));
            cboBlokKlant.Properties.Items.AddRange(klantenLijst.ToArray());
        }
        ...
        private void dateTimePicker1_ValueChanged(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            CboKlanten.Properties.Items.Clear();
            List<Factuur> facturenLijst = Factuur.KrijgAlleFacturenVanDatum(dateTimePicker1.Value.Date);
            ...
            foreach (Factuur factuur in facturenLijst)
            {
                ...
                if (klantenLijst.Exists(x => x.Naam == klant.Naam))
                {
        ...
        private void txtBericht_TextChanged(object sender, EventArgs e)
        {
        ...
        private void cboBlokKlant_KeyDown(object sender, KeyEventArgs e)
        {
          //  cboBlokKlant.DroppedDown = true;
        }
        ...
        private void cboBlokKlant_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void lbxFacturen_SelectedIndexChanged(object sender, EventArgs e)
        {
